...
(function ($, window, document) {
    ...
    Foundation.libs.section = {
        ...
        events: function () {
            ...
            $(self.scope)
            .on(
                'click.fndtn.section', click_title_selectors.join(","), function (e) {
            ...
            $(window)
            .on(
                'resize.fndtn.section', self.throttle(
                    function () {
                        self.resize(); }, 30
                )
            )
            ...
            $(document).on(
                'click.fndtn.section', function (e) {
        ...
        close_navs: function (except_nav_with_title) {
            ...
            if (except_nav_with_title.length > 0) {
                ...
                if (self.is_horizontal_nav(section) || self.is_vertical_nav(section)) {
                    ...
                    navsToClose = navsToClose.filter(
                        function () {
        ...
        resize: function (sections, ensure_has_active_region) {
            ...
            if ($(self.settings.section_selector).filter(
                function () {
                    return should_be_resized($(this), true); }
            ).length > 0
            ) {
                self.check_resize_timer = setTimeout(
                    function () {
                        self.resize(
                            sections.filter(
                                function () {
    ...
    $.fn.reflow_section = function (ensure_has_active_region) {
        ...
        self.throttle(
            function () {
                self.resize(section, ensure_has_active_region); }, 30
        )();
        return this;
    };
